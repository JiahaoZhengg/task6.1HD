...
function FindQuestion() {
  ...
  useEffect(() => {
    if (flag) {
      // Get a reference to the collection of articles in question
      const Q_A = collection(db, "Q_A");
      let questions = [];

      const q = query(Q_A, where("type", "==", 0));
      getDocs(q).then(result => {
        // Extracting values to data
